export class CircularPakingChart {
    ...
    initVis() {
        ...
        vis.cScale = d3.scaleOrdinal().domain(vis.group).range(d3.schemeTableau10);

        vis.size = d3.scaleLinear().domain([0, 100]).range([7, 50]); // circle will be between 7 and 55 px wide

        // create a tooltip
        vis.Tooltip = d3.select(vis.config.parentElement).append("div").style("opacity", 0).attr("id", "tooltip");

        vis.xScale = d3.scaleOrdinal().domain(vis.group).range([0, 40, 80, 120, 160, 200]);
    ...
    mousemove = (e, d) => {
        this.Tooltip.html("<u>" + d.name + "-" + d.parent + "</u>" + "<br>" + d.value + "%")
            .style("display", "block")
