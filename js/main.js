...
Promise.all(
    ...
    .then(function (result) {
        ...
        tendencyBar = new GroupeBarChart(
            { parentElement: "#tendency_bar", containerWidth: window.innerWidth * 0.6, containerHeight: window.innerHeight * 0.45, margin: { top: 10, right: 30, bottom: 20, left: 50 } },
            tendency,
            ["술, 술자리 모두 선호", "술은 선호하나 술자리는 비선호", "술은 비선호하나 술자리는 선호", "술과 술자리 모두 비선호"],
            ["세대", "세대별 음주 선호 성향 (%)"]
        );
        quantityBar = new GroupeBarChart(
            { parentElement: "#quantity_bar", containerWidth: window.innerWidth * 0.9, containerHeight: window.innerHeight * 0.45, margin: { top: 10, right: 30, bottom: 20, left: 50 } },
            quantity,
